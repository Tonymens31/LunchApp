...
//load User Details
(() => {
    mthdGet(getUserDetailsByUserIdAPI(userId))
        .done(function (data) {
            if (data.length > 0)
                //localStorage.setItem("userDetails", JSON.stringify(data[0]));
                $("#txtFullName").text(`${userDetail.firstName} ${userDetail.lastName}`);

            if (Number(data[0].iFirstTimeLogin) === 0) 
    ...
    //if (!userDetail.iFirstTimeLogin && userDetail.iFirstTimeLogin < 1)
    //    $("#mdlPassword").modal("show");
})();

document.querySelector('#btnChangePass').addEventListener('click', function () {
    ...
    let dataForm = {
        ...
        "strOldPassword": document.querySelector('#txtCurrentPassword').value,
        "strNewPassword": document.querySelector('#txtNewPassword').value,
        "strConfirmPassword": document.querySelector('#txtConfirmNewPassword').value
    };

    mthdPutOrPost(changePasswordAPI(), "POST", dataForm, "#mdlPassword");
});
